...
public class ExpFragTeacher extends Fragment {
    Button mise;
    RequestQueue mQueue;
    Context contex;
    AdapterTeacher Adapt;
    RecyclerView rv;
    private ArrayList<TeacherModel> list_exp;

    public ExpFragTeacher(){

    }
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState){
        View v = inflater.inflate(R.layout.frag_exp_teacher,container,false);
        rv= v.findViewById(R.id.rvexp);
    ...
    private void getExpTeacher() {
        ...
        String url="http://eniso.info/ws/core/wscript?s=Return(bean(%27academicProfile%27).findTeacherCvItemsBySection("+ActivityTeacher.idteacher+",2))";
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            for (int i=0;i<all.length();i++){
                                ...
                                list_exp.add(new TeacherModel("Titre",title,"Etablissement",etab,"Détails",details,"Mots_Clés",key));

                            }



                            Adapt=new AdapterTeacher(contex,list_exp);
                            rv.setAdapter(Adapt);
