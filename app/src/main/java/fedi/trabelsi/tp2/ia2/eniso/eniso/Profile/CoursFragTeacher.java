...
public class CoursFragTeacher extends Fragment {
    ...
    RecyclerView rv;
    private ArrayList<TeacherModel> list_cours;

    public CoursFragTeacher(){

    }
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState){
        View v = inflater.inflate(R.layout.frag_cours_teacher,container,false);
        rv= v.findViewById(R.id.rvcours);
    ...
    private void getCoursTeacher() {
        list_cours=new ArrayList<>();
        String url="http://eniso.info/ws/core/wscript?s=Return(bean(%27academicProfile%27).findTeacherCvItemsBySection("+ActivityTeacher.idteacher+",1))";
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            for (int i=0;i<all.length();i++){
                                ...
                                JSONObject camp =var.getJSONObject("company");
                                if (camp.has("name")){
                                    etab=camp.getString("name");
                                }
                                else {
                                    etab="??????";
                                }
                                ...
                                list_cours.add(new TeacherModel("Titre",title,"Etablissement",etab,"Détails",details,"Mots_Clés",key));
                            ...
                            Adapt=new AdapterTeacher(contex,list_cours);
                            rv.setAdapter(Adapt);
